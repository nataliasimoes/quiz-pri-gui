...
export const useQuestionStore = defineStore("question", () => {
  ...
  function resetState() {
    data.value = { status: "success", questions: questions, error: undefined };
  ...
  async function fetchQuestions(amount: number = 10) {
    try {
      // resetState();
      // count.value = amount;
      // const questionResponse = await $fetch<QuestionResponse>(
